    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        <div class="bg-white p-3 shadow rounded-lg md:col-span-2">
            <form x-data="{
                ...
                get shippingCountryState(){
                    const country = this.countries.find(c => c.code === this.shippinggAddress.country_code)
                    if(country && country.states){
                        return JSON.parse(country.states);
                ...
                <div class="grid grid-cols-2 gap-3 mb-3">
                    <x-input
                        type="text"
                        name="first_name"
                        value="{{old('fisrt_name',$customer->first_name)}}"
                    ...
                    <x-input
                        type="text"
                        name="last_name"
                        value="{{old('fisrt_name',$customer->last_name)}}"
                ...
                <div class="mb-3">
                    <x-input
                        type="text"
                        name="email"
                        value="{{old('fisrt_name',$customer->email)}}"
                ...
                <div class="mb-3">
                    <x-input
                        type="text"
                        name="phone"
                        value="{{old('fisrt_name',$customer->phone)}}"
                    ...
                    <div>
                        <x-input type="select"
                            ...
                            <template x-for="country of countries" :key="country.code">
                                <option :selected="country.code === billingAddress.country_code" :value="country.code" x-text="country.name"></option>
                            </template>
                        </x-input>
                    </div>
                    <div>
                        <template x-if="billingCountryStates">
                            ...
                            <template x-for="[code, state] of Object.entries(billingCountryStates)" :key="code">
                                <option :selected="code === billingAddress.state" :value="code" x-text="state"></option>
                            </template>
                        </x-input>
                        </template>
                        <template x-if="!billingCountryStates">
                ...
                <div class="flex justify-between mt-6 mb-2">
                    <h2 class="text-xl mt-6 font-semibold mb-2">Shipping Address</h2>
                ...
                <div>
                    <template x-if="shippingCountryStates">
                        ...
                        <template x-for="[code, state] of Object.entries(shippingCountryStates)" :key="code">
                            <option :selected="code === shippingAddress.state" :value="code" x-text="state"></option>
                        </template>
                    </x-input>
                    </template>
                    <template x-if="!shippingCountryStates">
            ...
            <form action="{{route('profile_password.update')}}" method="post">
                ...
                <div class="mb-3">
                    <x-input
                        ...
                        class="w-full focus:border-purple-600 focus:ring-purple-600 border-gray rounded" />
                </div>
                <div class="mb-3">
                    <x-input
                        type="password"
